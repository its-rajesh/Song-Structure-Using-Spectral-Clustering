...
def features(filename):
    ...
    # X_cqt = librosa.amplitude_to_db(X_cqt ** 2.0, ref_power=np.max)
    X_cqt = librosa.amplitude_to_db(X_cqt ** 2.0, ref=1.0)
    ...
    X_timbre = librosa.feature.mfcc(S=librosa.amplitude_to_db(X_melspec),
                                    n_mfcc=N_MFCC)
    ...
    #print("BEAT TIMES B4 ZIP\n", beat_times)

    beat_intervals = zip(beat_times[:-1], beat_times[1:]) #changed np.asarray(zip())
    #print("BEAT INTERVALS AFTER ZIP\n", np.array(list(beat_intervals)))
    ...
    return X_cqt, X_timbre, np.array(list(beat_intervals))
...
def label_rep_sections(X, boundaries, n_types):
    ...
    C = KMeans(n_clusters=n_types, tol=1e-8)

    labels = C.fit_predict(X_rep_stack.T)

    return list(zip(boundaries[:-1], boundaries[1:])), labels #list() is added
...
def fixed_partition(Lf, n_types):
    ...
    C = KMeans(n_clusters=n_types, tol=1e-10, n_init=100)
...
def lsd(X_rep, X_loc, beat_intervals, parameters):
    ...
    print(beat_intervals, MIN_SEG, MAX_SEG)
...
if __name__ == '__main__':
    ...
    print(X_cqt, X_timbre, beat_intervals)
